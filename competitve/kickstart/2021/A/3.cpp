...
int main() {
    ...
    int ans, R, C, M, L, N, K, F, W;
    v(v(int)) G;
    scmp(p(int, p(int, int)), cmp) Q;
    v(v(bool)) X;
    ...
    foa(t, 1, T + 1) {
        ...
        fo(i, R) {
            fo(j, C) {
                if(G[i][j] == M) {
                    Q.emplace(make_pair(G[i][j], make_pair(i, j)));
                }
            }
        }
        ...
        L = 0;
        while(!Q.empty()) {
            i = Q.begin()->second.first;
            j = Q.begin()->second.second;
            Q.erase(Q.begin());

            if(i > 0 && X[i - 1][j] == false) {
                if(G[i][j] - G[i - 1][j] > 1) {
                    K = G[i][j] - G[i - 1][j] - 1;
                    G[i - 1][j] += K;
                    L += K;
